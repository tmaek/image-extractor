...
#
# プロセス内フラグを使って同一プロセス中の再試行ループを防ぐ。
# Streamlit はユーザー操作のたびにスクリプトを再実行するため、
# インストール失敗時にマーカーが残らない場合でも再インストールが繰り返されないようにする。
_pw_marker = os.path.expanduser("~/.cache/ms-playwright/.installed")
_PW_INSTALL_ATTEMPTED = False  # モジュールスコープの試行フラグ（同一プロセス内で1回だけ実行）

if not os.path.exists(_pw_marker) and not _PW_INSTALL_ATTEMPTED:
...
def _is_private_host(host: str) -> bool:
    """True if host is a private/loopback/link-local/unspecified IP literal.
    0.0.0.0 and :: are also blocked as they can route to localhost in some environments."""
    try:
        ...
        return (
            ...
            or addr.is_unspecified  # 0.0.0.0 / :: を追加
        )
    except ValueError:
        return False
...
def _is_hostname_safe(hostname: str) -> bool:
    """DNS-based safety check for a hostname; result cached 1 min to reduce
    the window for DNS rebinding attacks (check-then-use gap cannot be fully
    eliminated without custom socket-level IP pinning in Python's requests)."""
...
def _is_safe_img_url(url: str, strict: bool = False) -> bool:
    """Safety check for image URLs.
    strict=False (listing): scheme + private IP literal check only.
    strict=True (fetching): full DNS resolution check."""
    try:
        p = urlparse(url)
        if p.scheme not in ("http", "https"):
            return False
...
def _safe_get(url: str) -> requests.Response:
    """Make a GET request following redirects, validating each hop for SSRF safety.
    Raises ValueError if any redirect destination is private/unsafe."""
    max_redirects = 10
    for _ in range(max_redirects):
        ...
        if r.is_redirect:
            ...
            if err:
                r.close()
                raise ValueError(f"Redirect blocked ({next_url}): {err}")
            url = next_url
            r.close()
            continue
        return r
...
def _download_bytes(url: str, max_bytes: int = MAX_RESPONSE_BYTES) -> Optional[bytes]:
    ...
    Uses _safe_get to validate each redirect hop (SSRF mitigation).
    Uses context manager to ensure the connection is properly closed even on early exit."""
    with _safe_get(url) as r:
        r.raise_for_status()
        chunks: list[bytes] = []
        total = 0
...
def fetch_thumbnail_cached(url: str) -> Optional[bytes]:
    ...
    Re-validates URL at fetch time to mitigate TOCTOU DNS-rebinding attacks."""
    if not _is_safe_img_url(url, strict=True):
        return None
    try:
        ...
        # ValueError is raised by _safe_get() when a redirect targets a private IP
        return None
...
def _fetch_html_playwright(page_url: str) -> str:
    ...
    Route interception blocks sub-resource requests to private/internal IPs (SSRF mitigation)."""
    with sync_playwright() as p:
        ...
        try:
            ...
            def _handle_route(route):
                """Abort requests targeting private/internal hosts."""
                try:
                    parsed = urlparse(route.request.url)
                    host = parsed.hostname or ""
                    if parsed.scheme in ("http", "https") and not _is_hostname_safe(host):
                        route.abort()
                        return
                except Exception:
                    pass
...
def fetch_image_urls(page_url: str, use_playwright: bool = False) -> list[dict]:
    ...
    def _add_url(raw_src: str) -> None:
        ...
        abs_url = parsed._replace(query="", fragment="").geturl()
    ...
    # Non-img elements with data-src (e.g. lazy-loaded divs with background-image)
    for tag in soup.find_all(attrs={"data-src": True}):
        if tag.name == "img":
            continue
        _add_url(tag.get("data-src") or "")

    # Inline style background-image: url(...)
    for tag in soup.find_all(style=True):
        for m in re.finditer(r'url\(["\']?(https?://[^"\')\s]+)["\']?\)', tag.get("style", "")):
            _add_url(m.group(1))
...
if images:
    ...
    with right_col:
        ...
        if n == 0:
            ...
            # ZIP は選択変更のたびに再描画で生成しないよう遅延評価する。
            # 「ZIPを準備する」ボタンが押されたときだけ生成し、session_state にキャッシュする。
            sel_key = hash(tuple(selected_imgs))
            if st.session_state.get("_zip_sel_key") != sel_key:
                # 選択が変わったら準備済みフラグをリセット
                st.session_state["_zip_sel_key"] = sel_key
                st.session_state["_zip_ready"] = False

            if not st.session_state.get("_zip_ready"):
                if st.button(
                    f"🔧 ZIPを準備する（{n} 枚）",
                    use_container_width=True,
                    type="primary",
                ):
                    with st.spinner("画像をダウンロードしてZIPを生成中…"):
                        zip_bytes, actual_count = make_zip_cached(tuple(selected_imgs))
                    st.session_state["_zip_cache"] = (zip_bytes, actual_count)
                    st.session_state["_zip_ready"] = True
                    st.rerun()
            else:
                zip_bytes, actual_count = st.session_state["_zip_cache"]
                count_label = (f"{actual_count}/{n} 枚" if actual_count < n else f"{n} 枚")
                st.download_button(
